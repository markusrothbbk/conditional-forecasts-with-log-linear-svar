...
max_forecast_horizon = 16;

%% (1) Example 1: conditional forecasts for a model specified in (variable) levels
% - example combines cases 1.a, 1.b, 2, 3.a, 3.b, 4.a, 5.a, 6.a of conditionsTable.xlsx.

fprintf("\n\n EXAMPLE 1: CONDITIONAL FORECASTS IN A VAR SPECIFIED IN LEVELS-VARIABLES \n\n") 
...
[c1,c1_draws,c1_table,c1_checks] = conditionalForecast(bb,chol(ss)',3,p,yy,xx,max_forecast_horizon,conditions1,1000); 
...
% information is collected automatically in c1_checks
    ...
    fprintf("\n\n Example 1: The following table checks if conditions hold in the actual forecast.\n\n") 
    disp([...
        array2table(["1.a"; "2"; "3.a"; "4.a"; "1.b"; "3.b"; "5.a"; "6.a"],"VariableNames","#"),...
        array2table(conditions1{:,"value"},"VariableNames","Condition"), ...
        array2table([tmp1a; tmp2; tmp3a; tmp4a; log(tmp1b); tmp3b; tmp5a; log(1+tmp6a)],"VariableNames","Value in actual forecast"), ...
        ])

% unconditional forecast
[u1_array,u1_draws,u1_table] = conditionalForecast(bb,chol(ss)',3,p,yy,xx,max_forecast_horizon,[],1000); 
u1_draws_p05 = quantile(u1_draws,0.05,3);
u1_draws_p95 = quantile(u1_draws,0.95,3);

% illustration
figure('Name','EXAMPLE 1: forecasts in (log-)levels','Position',[100,100,1200,600]);   
HOR = (0:max_forecast_horizon)';  
HOR_fill = [HOR;flipud(HOR)];  
for jj = 1:3  
    band_c_fill = [c1_draws_p05(:,jj); flipud(c1_draws_p95(:,jj))];  
    band_u_fill = [u1_draws_p05(:,jj); flipud(u1_draws_p95(:,jj))];  
    subplot(1,3,jj);  
    hold on  
    p_c = plot(HOR,c1(:,jj),'r-','LineWidth',1.5);  
    p_c_band = fill(HOR_fill,band_c_fill,'r','FaceAlpha',0.2,'EdgeColor','none');  
    p_u = plot(HOR,u1_array(:,jj),'b-','LineWidth',1.5);  
    p_u_band = fill(HOR_fill,band_u_fill,'b','FaceAlpha',0.2,'EdgeColor','none');  
    title(strcat("variable ",string(jj)))  
    xlabel("forecast horizon (h)")  
    xlim([HOR(1),HOR(end)])  
    grid on  
end
legend([p_u;p_u_band;p_c;p_c_band],...
    "Unconditional","Draws: 5%-95%","Conditional","Draws: 5%-95%",...
    'EdgeColor','none','Location','best')

%% (2) Example 2: conditional forecasts for a model specified in (variable) first-differences
% - example combines cases 4.b, 5.b, 6.b of conditionsTable.xlsx.

fprintf("\n\n EXAMPLE 2: CONDITIONAL FORECASTS IN A VAR SPECIFIED IN DIFFERENCED-VARIABLES \n\n") 
...
% obtain conditional forecast
[c2,c2_draws,c2_table,c2_checks] = conditionalForecast(dbb,chol(dss)',3,p,dyy,dxx,max_forecast_horizon,conditions2,1000); 
...
% information is collected automatically in c2_checks
    ...
    tmp6b = log(exp(c2_levels(9,3)) / exp(c2_levels(5,3)))/4;

    fprintf("\n\n Example 2: The following table checks if conditions hold in the actual forecast.\n\n") 
    disp([...
        array2table(["4.b"; "5.b"; "6.b"; "7"],"VariableNames","#"),...
        array2table(conditions2{:,"value"},"VariableNames","Condition"), ...
        array2table([tmp4b; tmp5b; tmp6b; 0],"VariableNames","Value in actual forecast"), ...
        ])
   
% unconditional forecast
[u2_array,u2_draws,u2_table] = conditionalForecast(dbb,chol(dss)',3,p,dyy,dxx,max_forecast_horizon,[],1000); 
u2_draws_p05 = quantile(u2_draws,0.05,3);
u2_draws_p95 = quantile(u2_draws,0.95,3);

% illustration
figure('Name','EXAMPLE 2: forecasts in differenced variables','Position',[100,100,1200,600]);   
HOR = (0:max_forecast_horizon)';  
HOR_fill = [HOR;flipud(HOR)];  
for jj = 1:3  
    band_c_fill = [c2_draws_p05(:,jj); flipud(c2_draws_p95(:,jj))];  
    band_u_fill = [u2_draws_p05(:,jj); flipud(u2_draws_p95(:,jj))];  
    subplot(1,3,jj);  
    hold on  
    p_c = plot(HOR,c2(:,jj),'r-','LineWidth',1.5);  
    p_c_band = fill(HOR_fill,band_c_fill,'r','FaceAlpha',0.2,'EdgeColor','none');  
    p_u = plot(HOR,u2_array(:,jj),'b-','LineWidth',1.5);  
    p_u_band = fill(HOR_fill,band_u_fill,'b','FaceAlpha',0.2,'EdgeColor','none');  
    title(strcat("variable ",string(jj)))  
    xlabel("forecast horizon (h)")  
    xlim([HOR(1),HOR(end)])  
...
figure('Name','EXAMPLE 2: forecasts transformed to (log-)levels','Position',[100,100,1200,600]); 
HOR = (0:max_forecast_horizon)';
HOR_fill = [HOR;flipud(HOR)];
...
for jj = 1:3
    ...
    % error bands 
    band_c_fill = [c2_draws_p05_levels(:,jj);flipud(c2_draws_p95_levels(:,jj))];
    band_u_fill = [u2_draws_p05_levels(:,jj);flipud(u2_draws_p95_levels(:,jj))];

    subplot(1,3,jj);
    hold on
    p_c = plot(HOR,c2_levels(:,jj),'r-');
    p_c_band = fill(HOR_fill,band_c_fill,'r','FaceAlpha',0.2,'EdgeColor','none');
    p_u = plot(HOR,u2_levels(:,jj),'b-');
    p_u_band = fill(HOR_fill,band_u_fill,'b','FaceAlpha',0.2,'EdgeColor','none');
    title(strcat("variable ",string(jj)))
    xlabel("forecast horizon (h)")
    xlim([HOR(1),HOR(end)])
